...
class VolunteerGroupJoinedFragment : Fragment() {
    var binding:FragmentVolunteerGroupJoinedBinding?=null
    private var list=ArrayList<HashMap<String,String>>()
    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        binding= FragmentVolunteerGroupJoinedBinding.inflate(layoutInflater)
        return binding!!.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        getInformation()
        //列表没想监听
        binding!!.volunteerGroupJoinedList.setOnItemClickListener { parent, view, position, id ->
            ...
            view.findViewById<TextView>(R.id.group_tab).setOnClickListener {
                ...
                dialog.findViewById<TextView>(R.id.dialog_image).text="脱离"
                dialog.findViewById<TextView>(R.id.dialog_image).setOnClickListener {
                    val db=MyDBHelper(requireContext()).writableDatabase
                    if (db.delete("volunteer_group","v_id=? and g_id=?", arrayOf(id.toString(),gid))>0) getInformation() else Toast.makeText(requireContext(),"删除失败",Toast.LENGTH_SHORT).show()
                    dialog.dismiss()
                }
    ...
    fun getInformation(){
        list.clear()
        val id=requireContext().getSharedPreferences("UserInfo", Context.MODE_PRIVATE).getString("userId","")?.split("-")?.get(0)
        val sql="select `g_name`,`principal_name`,`register_date`,`group`.`id` as `gId` from `group`,`volunteer_group` where `group`.`id`=g_id and `v_id`=? and `state`='已通过'"
        ...
        binding!!.volunteerGroupJoinedList.adapter=adapter
    }
}