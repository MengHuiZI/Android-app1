...
import com.example.myapplication.databinding.FragmentVolunteerGroupApplyingBinding


class VolunteerGroupApplyingFragment : Fragment() {
    var binding:FragmentVolunteerGroupApplyingBinding?=null
    ...
    override fun onCreateView(
        ...
        binding= FragmentVolunteerGroupApplyingBinding.inflate(layoutInflater)
        return binding!!.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        getInformation()
        //列表没想监听
        binding!!.volunteerGroupApplyingList.setOnItemClickListener { parent, view, position, id ->
    ...
    fun getInformation(){
        list.clear()
        val id=requireContext().getSharedPreferences("UserInfo", Context.MODE_PRIVATE).getString("userId","")?.split("-")?.get(0)
        val sql="select `g_name`,`principal_name`,`register_date`,`group`.`id` as `gId` from `group`,`volunteer_group` where `group`.`id`=g_id and `v_id`=? and `state`='待审核'"
        ...
        binding!!.volunteerGroupApplyingList.adapter=adapter
    }
}