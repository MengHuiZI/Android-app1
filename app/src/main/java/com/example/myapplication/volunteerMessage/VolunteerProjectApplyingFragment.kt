...
import com.example.myapplication.databinding.FragmentVolunteerProjectApplyingBinding


class VolunteerProjectApplyingFragment : Fragment() {

    var binding:FragmentVolunteerProjectApplyingBinding?=null
    ...
    override fun onCreateView(
        ...
        binding=FragmentVolunteerProjectApplyingBinding.inflate(layoutInflater)
        return binding!!.root
    }
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        getInformation()
        //列表没想监听
        binding!!.volunteerProjectApplyingList.setOnItemClickListener { parent, view, position, id ->
            ...
            view.findViewById<TextView>(R.id.group_tab).setOnClickListener {
                ...
                dialog.findViewById<TextView>(R.id.dialog_image).text="取消申请"
    ...
    fun getInformation(){
        list.clear()
        val id=requireContext().getSharedPreferences("UserInfo", Context.MODE_PRIVATE).getString("userId","")?.split("-")?.get(0)
        val sql="select `p_name`,`p_start_time`,`project`.`id` as `pId`,`p_service_time` from `project`,`volunteer_project` where `project`.`id`=p_id and `v_id`=? and `state`='待审核'"
        ...
        binding!!.volunteerProjectApplyingList.adapter=adapter
    }
}